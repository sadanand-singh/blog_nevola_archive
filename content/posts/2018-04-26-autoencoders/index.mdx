...
constraints) along with a sample implementation of it using [Keras][keras],
with a [tensorflow] back-end.

[keras]: https://keras.io
[tensorflow]: https://tensorflow.org

> Please note that this post has been written using Tensorflow 1.x version of Keras!
...
- $W_i \in \mathbb{R}^{I_{di} \times O_{di}}$, the parameter matrix
  of $i$-th layer, projecting a $I_{di}$ dimensional input in
  a $O_{di}$dimensional space
...
**number of hidden units** ($z$) should be less than the dimensions ($d$) of
the input ($z < d$).
...
such as [PCA][pca] due to the non-linearity and the type of constraints applied.

> **PCA and Autoencoders** <br/><br/>If we were to construct a linear network (i.e. without the use of nonlinear activation functions at each layer) we would observe a similar dimensionality reduction as observed in [PCA][pca]. See [Geoffrey Hinton's discussion][1].
...
<div className='Image__Medium'>
  <img
    src='https://cdn-images-1.medium.com/max/1600/1*op0VO_QK4vMtCnXtmigDhA.png'
    alt='AE diagram'
  />
</div>
...
I prefer to use [fashion MNIST][f-mnist] dataset for the [reasons described here][2].
...
For example using [MNIST][mnist] data, please have a look at the [article] by
...
![](https://res.cloudinary.com/sadanandsingh/image/upload/v1544987534/images/autoencoders/ae_sparsity_fm.png)
...
is the encoder. A Denoising autoencoders (DAE) instead minimizes, $L\Big(x, g\big(f(\hat{x})\big)\Big)$
...
decoder to implicitly learn the structure of $p_{data}(x),$ the
...
- **Dimensionality Reduction**: Dimensionality reduction was one of the first
  ...
  techniques like [PCA][pca], [kernel PCA][kpca] etc.
